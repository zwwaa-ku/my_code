...
for repeat in range(counter):
    ...
    x_train,y_train = mk_data(x_all[:boader], y_all[:boader], split, split_use_num, swap_num)
    ...
    if repeat==0:
        ...
        open(os.path.join('../data/',directoryname[fivefactor],filestr,jsonfilename), 'w').write(json_string)
    
    hdf5filename=str(repeat)+'cnn_model_weights.hdf5'
    model.save_weights(os.path.join('../data/',directoryname[fivefactor],filestr,hdf5filename))
...
np.savetxt('../data/'+directoryname[fivefactor]+'/'+filestr+'/indexlist.csv', indexlist, delimiter=",", fmt='%d')

with open('../data/'+ directoryname[fivefactor]+'/'+filestr+'/cov.csv', 'w') as j:
    writer = csv.writer(j)
    writer.writerows(covlist)
with open('../data/'+ directoryname[fivefactor]+'/'+filestr+'/score.csv', 'w') as f:
    writer = csv.writer(f)
